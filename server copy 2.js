...
app.get('/success', async (req, res) => {
  ...
  try {
    ...
    console.log('session_id server.js success:');//------------------------------------------------log
...
app.post('/printful/stores', async (req, res) => {
  console.log('POSTE:');//------------------------------console log
  
  //console.log('session_id server.js:');//------------------------------------------------log
  //const { session_id } = req.query; // Obtenha o ID da sessão do Stripe
...
app.post('/checkout', async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: process.env.CLIENT_URL + "/success?session_id={CHECKOUT_SESSION_ID}", // Use {CHECKOUT_SESSION_ID} como marcador de substituição
      cancel_url: process.env.CLIENT_URL + "?success=false",
    });

    res.json({ checkoutUrl: session.url, session_id: session.id }); // Retorne o session_id
  } catch (error) {
...
const criarPedidoNaPrintful = async (stripeSession) => {
  console.log('PEDIDO PRINTFUL');//------------------------------log
